---
name: minions-sprint-master
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Sprint-master MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-sprint-master data."
category: ai
subcategory: general
...
# Minions Sprint-master Agent Skills

Skills for agents operating on the `minions-sprint-master` toolbox.
...
```bash
# TypeScript
pnpm add @minions-sprint-master/sdk

# Python
pip install minions-sprint-master

# CLI
pnpm add -g @minions-sprint-master/cli
```
...
The `sprint-master` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-sprint-master/sdk/schemas';
...
```python
from minions_sprint_master.schemas import custom_types
...
- This agent only writes to `minions-sprint-master` — it reads from other toolboxes but never writes to them